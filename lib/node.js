...
var KeyIndex = require('./keyindex').KeyIndex;
...
var Node = function Node(cfg) {
  ...
  this.keyIndex = new KeyIndex(this.storage, this.blockChain);
...
Node.prototype.getKeyIndex = function () {
  ...
  e.message.invs.forEach(function (inv) {
    ...
    case 2: // MSG_BLOCK
      self.blockChain.getBlockByHash(inv.hash, function (err, block) {
        ...
        self.storage.Transaction.find({block: inv.hash}, function (err, txs) {
          if (err) {
