 ...
 */
Block.method('getNextWork', function (blockChain) {
  ...
  var lastBlock = blockChain.getBlockByHeight(this.height - interval + 1);

  // Determine how long the difficulty period really took
  var actualTimespan = this.timestamp - lastBlock.timestamp;

  // There are some limits to how much we will adjust the difficulty in one step
  ...
  return Util.encodeDiffBits(newTarget);
});
