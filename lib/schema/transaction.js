...
Transaction.prototype.getStandardizedObject = function getStandardizedObject() {
  ...
  var ins = this.ins.map(function (txin) {
    ...
    if (txin.isCoinBase()) {
      ...
      txinObj.scriptSig = new Script(txin.script).toString(false, 0);
    }
    totalSize += 36 + txin.script.length + 4; // outpoint + script + sequence
    return txinObj;
  });
  ...
  var outs = this.outs.map(function (txout) {
    var txoutObj = {
      ...
      scriptPubKey: new Script(txout.script).toString(false, 0)
    };
  });
