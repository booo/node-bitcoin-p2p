var mongoose = require('bitcoinjs-mongoose'); // database
...
var MongoBinary = require('bitcoinjs-mongoose').Types.Buffer.Binary;
...
var TransactionIn = exports.TransactionIn = function TransactionIn(data) {
  ...
  this.script = Buffer.isBuffer(data.script) ? data.script : new Buffer(0);
...
Transaction.prototype.verify = function verify(txCache, callback) {
  ...
  try {
    ...
    self.ins.forEach(function (txin, n) {
      ...
      valueIn = valueIn.add(Util.valueToBigInt(txout.value));

      srcOutCondList.push({
        "ins.outpoint.hash": new MongoBinary(txin.outpoint.hash, 0x00),
