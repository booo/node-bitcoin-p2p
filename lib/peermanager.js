...
PeerManager.prototype.checkStatus = function checkStatus()
{
  ...
  if (this.peers.length < this.minKnownPeers) {
    ...
    while (bootstrap.length > 0) {
      var bso = bootstrap.shift();
      if ("function" == typeof bso.bootstrap) {
        bso.bootstrap(this.node, this);
      } else {
        logger.warn("Invalid bootstrapping module, possible error in settings.");
      }
    }
  }
