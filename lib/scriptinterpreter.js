...
ScriptInterpreter.prototype.eval = function eval(script, tx, inIndex, hashType, callback)
{
  ...
  console.log(script.toString(false, 0));
  ...
  function executeStep(cb) {
    ...
    if (pc >= script.chunks.length) {
      ...
      // final stack contains a single value of 1.)
      cb(null);
      return;
    }
    ...
    try {
      ...
      case OP_CHECKSIGVERIFY:
        ...
        var success;
        try {
          success = checkSig(sig, pubkey, scriptCode, tx, inIndex, hashType);
        } catch(e) {
          success = false;
        }
      ...
      case OP_CHECKMULTISIGVERIFY:
        ...
        while (success && sigsCount > 0) {
          ...
          try {
            if (checkSig(sig, key, scriptCode, tx, inIndex, hashType)) {
              isig++;
              sigsCount--;
            }
          } catch(e) {}
