...
var BlockChain = exports.BlockChain = function BlockChain(storage, settings) {
    ...
    Step(
      ...
      function prepareTxs(err) {
        ...
        bw.txs = bw.txs.map(function (tx) {
          if (!((tx instanceof PlainTransaction) || (tx instanceof Transaction))) {
            tx = new PlainTransaction(tx);
          }
          // Calculate the hash
          tx.getHash();
        ...
        bw.txs.forEach(function (tx, i) {
          ...
          tx.cacheInputs(self, localTx, true, function (err, txCache) {
            ...
            if (i === 0 && tx.isCoinBase()) {
              // Coinbase transactions aren't verified
              callback(null);
            } else {
  ...
  var saveBlock = this.saveBlock = function saveBlock(bw, callback)
  {
    ...
    storage.saveBlock(dbObj, function (err) {
      if (err) {
        callback(err);
        return;
      }
  ...
  function saveTransactions(block, txs, callback) {
    ...
    txs.forEach(function (tx, i) {
      ...
      // TODO: Find a faster way than using Mongoose.
      dbTxs.push(new Transaction(tx).toObject());
    ...
    storage.saveTransactions(dbTxs, function (err) {
      if (err) {
        callback(err);
        return;
      }
        ...
        reorgSteps.push(function (err) {
          ...
          self.getTransactionsByHashes(block.txs, function (err, txs) {
            ...
            revokeSteps.push(function () {
              ...
              storage.saveBlock(block.toObject(), this);
            });
