...
var BlockChain = exports.BlockChain = function BlockChain(storage, settings) {
  ...
  this.getBlockByHash = function getBlockByHash(hash, callback) {
    Block.findOne({hash: hash}, callback);
  };
  ...
  this.getTransactionByHash = function (hash, callback) {
    Transaction.findOne({hash: hash}, callback);
  };
  ...
  this.addTransactions = function addTransactions(block, txs) {
    txs.forEach(function (tx, i) {
      ...
      Transaction.collection.update(
        ...
        // Data
        tx.toObject(),
        // Update if exists, this means that we will consider the latest
        // transaction with that hash valid, which matches the behavior of the
        // original client.
        { upsert : true },
        // Callback
        handleTxInsertResult.bind(self, block, i, tx)
      );
    });
  };

  var handleTxInsertResult = function (block, i, tx, err) {
    if (err) {
      logger.warn(err);
      return;
    }
  ...
  this.getBlockByLocator = function (locator, callback) {
    ...
    Block.find({hash: {"$in": locator}, active: 1}).sort('height', -1).limit(1).exec(function (err, result) {
      if (err) {
        callback(err);
        return;
      }
