...
var BlockChain = exports.BlockChain = function BlockChain(storage, settings) {
    ...
    Step(
      ...
      function applyParent(err) {
        ...
        case 'side':
          ...
          break;
        case 'orphan':
          // "Magic" exception which causes us to skip to the end
          throw "orphan";
          break;
        }
      },
      ...
      function save(err) {
        ...
        saveBlock(bw, this.parallel());
        saveTransactions(bw.block, bw.txs, this.parallel());
  ...
  function connectBlock(bw, callback) {
    ...
    } else if ((parent = recentBlockIndex.getByHash(bw.parent64))) {
      // Unwrap
      parent = parent.block;
...
RecentBlockIndex.prototype.add = function add(bw) {
  this.list.push(bw);
  this.index[bw.hash64] = bw;
...
RecentBlockIndex.prototype.clean = function clean() {
  while (this.list.length > this.maxEntries) {
    ...
    delete this.index[dropped.hash64];
  }
};
