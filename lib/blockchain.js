...
var BlockChain = exports.BlockChain = function BlockChain(storage, settings) {
  ...
  var add = this.add = function add(block, txs, callback) {
    ...
    txs = txs.map(function (tx) {
      ...
      // These two calls are caching, we just trigger them so the information is
    ...
    // Create and store tx reference list
    ...
    if (bw.block.moreWorkThan(currentTopBlock)) {
      saveBlock(bw, function (err) {
        ...
        self.reorganize(currentTopBlock, bw.block, function (err) {
          ...
          console.log("children",bw.children && bw.children.length);
          if (bw.children) {
            bw.children.forEach(function (childBw) {
              childBw.block.attachTo(bw.block);
              connectToMainChain(childBw);
...
BlockWrapper.prototype.getCallback = function getCallback() {
  var self = this;
  return function (err) {
    if (err) {
