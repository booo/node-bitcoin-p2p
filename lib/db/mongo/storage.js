...
function MongoStorage(uri) {
  ...
  this.saveBlock = function (block, callback) {
    ...
    Block.collection.insert(blockObj, {safe: true}, function (err) {
      try {
        ...
        if (err && err.message.indexOf("E11000") !== -1) {
          Block.collection.update(
            {_id: block._id}, blockObj, {upsert: true}, function (err) {
              try {
                callback(err);
              } catch (err) {
  ...
  this.saveTransactions = function (txs, callback) {
    ...
    txs = txs.map(function (tx) {
      return new Transaction(tx).toObject();
    });
  ...
  function getTransactionByHash(hash, callback) {
    Transaction.findOne({_id: hash}, function (err, result) {
      try {
        if (result) {
          result = new PlainTransaction(result);
        }
        callback(err, result);
      } catch (err) {
        logger.error('Storage: Uncaught callback error: ' +
  ...
  this.getBlocksByHeights = function (heights, callback) {
    Block.find({height: {$in: heights}}, function (err, result) {
      try {
        ...
        result = result.map(function (block) {
          return new PlainBlock(block);
        });
  ...
  function getBlocksByHashes(hashes, callback) {
    var cursor = Block.find({_id: {$in: hashes}}, function (err, results) {
        try {
          if (err) {
            callback(err);
            return;
  ...
  function getBlockByHeight(height, callback) {
    Block.findOne({height: height, active: true}, function (err, result) {
      try {
        callback(err, new PlainBlock(result));
      } catch (err) {
        logger.error('Storage: Uncaught callback error: ' +
                     (err.stack ? err.stack : err.toString()));
      }
  ...
  function getTopBlock(callback) {
    Block
      ...
      .exec(function (err, result) {
        ...
        callback(null, new PlainBlock(result[0]));
      });
  };
