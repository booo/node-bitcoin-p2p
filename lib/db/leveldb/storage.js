...
function LeveldbStorage(uri) {
      ...
      function saveAffects(err) {
        ...
        txs.forEach(function (tx) {
          ...
          tx.affects.forEach(function (affect) {
            wb.put(affect.concat(tx.getHash()), '');
          });
        });
  ...
  function getAffectedTransactions(addrHash, callback)
  {
    var addrHash64 = addrHash.toString('base64');
    var iterator = bTxAffectsIndex.newIterator({});
    Step(
      function rewindIteratorStep() {
        iterator.seek(addrHash, this);
      },
      function iterateStep(err) {
        if (err) throw err;
