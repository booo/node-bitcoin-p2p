...
var BlockChainManager = exports.BlockChainManager = function (blockChain, peerManager) {
  ...
  this.lastSeenTop = Util.EMPTY_BUFFER;
...
util.inherits(BlockChainManager, events.EventEmitter);
...
BlockChainManager.prototype.startDownload = function (toHash, fromHash, conn)
{
  ...
  try {
    // Don't start a new download while one is still going
    if (this.currentDownload) {
      return;
    }
...
util.inherits(BlockChainDownload, events.EventEmitter);
...
BlockChainDownload.prototype.start = function start()
{
  ...
  this.blockChain.getBlockLocator((function (err, locator) {
    ...
    // We use this heuristic to guess whether this is the initial download or
    // not. It only influences the log message, so not too tragic if we're
    // wrong.
    if (topBlock.height < this.conn.bestHeight) {
      // Probably part of the initial download
      ...
      // The initial download is probably done
      logger.info('Checking for blocks '+
                  '> '+curHeight +
                  blockMeta);
    }
